...
function getSurfaceDiff(previous, current) {
  if (!previous) {
    return {
      firstSeen: true,
      statusChanged: true,
  ...
  return {
    ...
    finalUrlChanged: previous.finalUrl !== current.finalUrl,
    etagChanged: (previous.etag || "") !== (current.etag || ""),
    lastModifiedChanged: (previous.lastModified || "") !== (current.lastModified || ""),
    contentLengthChanged:
      String(previous.contentLength || "") !== String(current.contentLength || ""),
    htmlHashChanged: (previous.htmlHash || "") !== (current.htmlHash || ""),
  };
}
...
function analyzeChanges(previousState, currentResults) {
  ...
  for (const current of currentResults) {
    ...
    const diff = getSurfaceDiff(previous, current);
    ...
    if (signatureChanged) {
      ...
      maxPriority = Math.max(maxPriority, SURFACE_PRIORITY[current.label] || 1);
    }
    ...
    if (newKeywords.length > 0) {
      ...
      maxPriority = Math.max(maxPriority, 7);
    }
  }
...
async function main() {
  ...
  const nextState = {
    ...
    maxPriority: analysis.maxPriority,
  ...
  if (!previousSignature) {
    await appendSentinelEvent({
      ...
      maxPriority: 0,
      threshold: {
        hasCandidateActivation: false,
        hasKeywordTrigger: false,
        highPrioritySurfaceChanged: false,
        multipleSurfaceChange: false,
      },
    });
  ...
  const highPrioritySurfaceChanged = analysis.maxPriority >= 3;
  const multipleSurfaceChange = analysis.changedSurfaces.length >= 2;
  const hasKeywordTrigger = analysis.keywordTriggers.length > 0;
  const hasCandidateActivation = analysis.activatedCandidates.length > 0;

  const shouldTriggerDeepScan =
    currentSignature !== previousSignature &&
    (
      hasCandidateActivation ||
      hasKeywordTrigger ||
      highPrioritySurfaceChanged ||
      multipleSurfaceChange
    );
