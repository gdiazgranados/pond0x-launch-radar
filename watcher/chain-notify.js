...
async function main(){
 ...
 const w=c.windows?.['5m']||{}, p=c.predictor||{}, a=c.cycleAnalytics||{}, m=c.patternMatch||{}; const now=Date.now(); const lastSent=prev.sentAt?new Date(prev.sentAt).getTime():0; const mins=(now-lastSent)/60000;
 ...
 if(!(resumed||spike||funding||cycle||fundingWindow||patternCross||periodic)){console.log('No material chain alert');return}
 ...
 if(await send(msg)){await fs.writeJson(stateFile,{sentAt:new Date().toISOString(),activityState:c.activityState,fundingDetected:c.fundingDetected,rewards5m:w.rewards,volume5m:w.wpondDistributed,cycleSignal:a.cycleSignal,predictorStatus:p.status,patternMatchPct:m.historicalPatternMatchPct,patternMatchStatus:m.status},{spaces:2}); console.log('Chain alert sent')}
}
