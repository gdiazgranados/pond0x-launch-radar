...
function eventId(event) {
  return [event.type, event.domain, event.subject || "", event.seenAt || ""].join("|");
}
...
async function main() {
  ...
  for (const [name, flag] of Object.entries(currentFlags)) {
    ...
    nextFlags[name] = {
      ...
      value: currentValue,
      lastSeenAt: checkedAt,
      lastTransitionAt: changed ? checkedAt : previous.lastTransitionAt || null,
      transitionCount: n(previous.transitionCount) + (changed ? 1 : 0),
    };

    if (changed) {
      const event = {
        type: flagTransitionType(name, previous.value, currentValue),
        domain: "featureFlag",
        subject: name,
        seenAt: checkedAt,
        snapshotId,
        ...
        detail: `${name}: ${previous.value} -> ${currentValue}`,
      };
      pushUnique(newEvents, event, seenIds);
      allEvents.push({ id: eventId(event), ...event });
    }
  }
  ...
  for (const transition of ensureArray(routeApi?.dormantToLive)) {
    ...
    const event = {
      type: isApi ? "API_DORMANT_TO_LIVE" : "ROUTE_DORMANT_TO_LIVE",
      domain: isApi ? "api" : "route",
      subject: transition.route,
      seenAt: transition.lastSeenAt || routeApi.checkedAt || checkedAt,
      snapshotId,
      status: transition.lastStatus ?? null,
      detail: `${transition.kind || "ROUTE"} ${transition.route} became live`,
    };
    pushUnique(newEvents, event, seenIds);
    allEvents.push({ id: eventId(event), ...event });
  }
  ...
  for (const transition of ensureArray(routeApi?.liveToDormant)) {
    const isApi = transition.kind === "API";
    const event = {
      type: isApi ? "API_LIVE_TO_DORMANT" : "ROUTE_LIVE_TO_DORMANT",
      domain: isApi ? "api" : "route",
      subject: transition.route,
      seenAt: transition.lastSeenAt || routeApi.checkedAt || checkedAt,
      snapshotId,
      status: transition.lastStatus ?? null,
      detail: `${transition.kind || "ROUTE"} ${transition.route} became dormant`,
    };
    pushUnique(newEvents, event, seenIds);
    allEvents.push({ id: eventId(event), ...event });
  }

  const featureSurfaceDrift = manifest?.featureSurfaceDrift || {};
  const bundleDiff = featureSurfaceDrift?.bundleDiff || {};
  const semanticSignals = [
    ...ensureArray(bundleDiff.addedApiRoutes),
    ...ensureArray(bundleDiff.addedKeywords),
    ...ensureArray(bundleDiff.addedFlags),
  ];
  if (bundleDiff.status === "DRIFT" && semanticSignals.length > 0) {
    const event = {
      type: "SEMANTIC_MATERIAL_CHANGE",
      domain: "semantic",
      subject: "first-party-bundles",
      seenAt: checkedAt,
      snapshotId,
      detail: semanticSignals.slice(0, 8).join(", "),
    };
    pushUnique(newEvents, event, seenIds);
    allEvents.push({ id: eventId(event), ...event });
  }
  ...
  const chainSeenAt =
    recipients?.lastObservedAt ||
    chain?.recipientLedger?.lastObservedAt ||
    chain?.recentExternalClaims?.[0]?.time ||
    chain?.recentExternalClaims?.[0]?.timestamp ||
    chain?.generatedAt ||
    checkedAt;

  if (newTransfers > 0) {
    const event = {
      type: "DISTRIBUTOR_TRANSFER",
      domain: "distributor",
      subject: chain?.entities?.claimDistributor || null,
      seenAt: chainSeenAt,
      snapshotId,
      count: newTransfers,
      detail: `${newTransfers} new external distributor transfer(s) observed`,
    };
    pushUnique(newEvents, event, seenIds);
    allEvents.push({ id: eventId(event), ...event });
  }
  ...
  if (newRecipients > 0) {
    const event = {
      ...
      snapshotId,
      count: newRecipients,
      detail: `${newRecipients} new recipient(s) observed`,
    };
    pushUnique(newEvents, event, seenIds);
    allEvents.push({ id: eventId(event), ...event });
  }
  ...
  if (rewardTransfers5m > 0 || fundingActive) {
    const event = {
      ...
      domain: "onchain",
      subject: null,
      seenAt: chain?.generatedAt || checkedAt,
      snapshotId,
      detail: fundingActive
        ? `Funding active; reward transfers 5m=${rewardTransfers5m}`
        : `Reward transfers 5m=${rewardTransfers5m}`,
    };
    pushUnique(newEvents, event, seenIds);
    allEvents.push({ id: eventId(event), ...event });
  }

  const deduped = [...new Map(allEvents.map((event) => [event.id || eventId(event), event])).values()]
    .sort((a, b) => new Date(b.seenAt).getTime() - new Date(a.seenAt).getTime())
    .slice(0, MAX_EVENTS);

  const nowMs = new Date(checkedAt).getTime();
  const recentEvents = deduped.filter((event) => {
    const ts = new Date(event.seenAt).getTime();
    return Number.isFinite(ts) && nowMs - ts >= 0 && nowMs - ts <= RECENT_WINDOW_MS;
  });
  ...
  const classification =
    activationLike && distributionLike && recentDomains.length >= 4
      ? "COORDINATED_ACTIVATION_CANDIDATE"
      : activationLike && recentDomains.length >= 3
        ? "ACTIVATION_SEQUENCE"
        : activationLike
          ? "SURFACE_TRANSITION"
          : recentDomains.length >= 3
  ...
  const result = {
    ...
    windowMinutes: 60,
    flagHistory: {
      observed: Object.keys(nextFlags).length,
      transitionsThisSweep: newEvents.filter((event) => event.domain === "featureFlag").length,
      states: nextFlags,
    },
    dormantActive: {
      routeTransitionsThisSweep: ensureArray(routeApi?.dormantToLive).filter((x) => x.kind === "ROUTE").length,
      apiTransitionsThisSweep: ensureArray(routeApi?.dormantToLive).filter((x) => x.kind === "API").length,
      deactivationsThisSweep: ensureArray(routeApi?.liveToDormant).length,
    },
    recent: {
      eventCount: recentEvents.length,
      domainCount: recentDomains.length,
      domains: recentDomains,
      events: recentEvents.slice(0, 60),
    },
    newEvents: newEvents.sort((a, b) => new Date(a.seenAt).getTime() - new Date(b.seenAt).getTime()),
    timeline: deduped.slice(0, 120),
    caution: "Activation timeline is observational correlation. A transition or temporal sequence does not prove causality, product launch, or reward eligibility.",
  };
  ...
  console.log(
    `Activation Timeline v2 | class=${classification} score=${recentScore} domains=${recentDomains.length} newEvents=${newEvents.length}`
  );
}

main().catch((error) => {
  console.error("activation-timeline failed:", error);
  process.exit(1);
});
