...
const ADDITIONAL_SIGNER = address(
...
function wire(options: {
  ...
  const accounts = options.additionalSigner
    ? [
        {
          address: ADDITIONAL_SIGNER,
  ...
  const message = pipe(
    ...
    value =>
      setTransactionMessageFeePayer(TAKER, value),
    value =>
      setTransactionMessageLifetimeUsingBlockhash(
        {
          blockhash: blockhash(
...
test(
  "captures a watch-only unsigned order for the pinned taker",
  async () => {
    let requested = ""
...
test(
  "blocks a wire whose fee payer differs from the taker",
  async () => {
...
test(
  "blocks a wire requiring an additional signer",
  async () => {
    const result = await request(async () =>
      new Response(
        JSON.stringify({
    ...
    assert.deepEqual(
      ...
      [TAKER, ADDITIONAL_SIGNER]
    )
    assert.ok(
      result.blockingReasons.includes(
        "PONDOX_ORDER_REQUIRED_SIGNERS_MISMATCH"
      )
    )
    assert.equal(result.signingEnabled, false)
    assert.equal(result.transactionSubmitted, false)
