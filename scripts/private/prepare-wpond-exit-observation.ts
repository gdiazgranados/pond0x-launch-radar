...
const SNAPSHOT_URL = "https://raw.githubusercontent.com/gdiazgranados/pond0x-launch-radar/radar-data/data/token-market-latest.json"
const TRENDS_URL = "https://raw.githubusercontent.com/gdiazgranados/pond0x-launch-radar/radar-data/data/token-market-trends-latest.json"
...
async function verifyDecimals() {
  ...
  if (Number(payload.result?.value?.decimals) !== 3) {
    throw new Error("unexpected on-chain wPOND decimals")
  }
}

function feeLamports() {
  const value = Number(process.env.SHADOW_SOL_FEE_LAMPORTS ?? 10_000)
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("SHADOW_SOL_FEE_LAMPORTS must be non-negative")
  }
  return value
}
...
async function main() {
  ...
  if (!positionId?.trim()) throw new Error("SHADOW_POSITION_ID is required")

  const dataDirectory = resolve(process.env.SHADOW_DATA_DIR ?? "private-data")
  ...
  const [state, basisLedger, snapshot, trends] = await Promise.all([
  ...
  if (!basis) throw new Error("LEGACY_EXECUTION_BASIS_UNAVAILABLE")
  ...
  const archiveKey = evaluatedAt.replace(/[^0-9A-Za-z]/g, "")
  const archivePath = resolve(
    dataDirectory,
    "exits",
    `${positionId}-${archiveKey}.json`
  )
  const envelope = {
    evaluatedAt,
    positionId,
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : "wPOND exit quote failed")
  process.exitCode = 1
})
