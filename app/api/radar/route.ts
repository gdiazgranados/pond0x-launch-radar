...
export async function GET() {
  try {
    const files = ["latest.json","history.json","heartbeat.json","sentinel-state.json","sentinel-events.json","chain-intelligence.json","chain-baseline.json","reward-recipients.json","system-health.json","telegram-health.json","distributor-intelligence.json","mining-intelligence.json","route-api-intelligence.json","activation-timeline.json","activation-decision.json","historical-replay.json","calibration-report.json","ground-truth-events.json","historical-evidence-archive.json","threshold-drift-report.json","pre-event-signature-intelligence.json","evidence-ledger.json","alerts-history.json","clear-intelligence.json"]
    const [latest,history,heartbeat,sentinelState,sentinelEvents,chainIntelligence,chainBaseline,rewardRecipients,systemHealth,telegramHealth,distributorIntelligence,miningIntelligence,routeApiIntelligence,activationTimeline,activationDecision,historicalReplay,calibrationReport,groundTruthEvents,historicalEvidenceArchive,thresholdDriftReport,preEventSignatureIntelligence,evidenceLedger,alertsHistory,clearIntelligence] = await Promise.all(files.map(loadRemoteJson))
